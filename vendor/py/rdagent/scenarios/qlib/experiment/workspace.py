...
class QlibFBWorkspace(FBWorkspace):
    ...
    def execute(self, qlib_config_name: str = "conf.yaml", run_env: dict = {}, *args, **kwargs) -> str:
        # lianghua 本地补丁：local_qlib 镜像内 mlflow 3.x 默认禁用文件存储后端，
        # 显式 opt-out（否则 qrun 与 read_exp_res.py 都会抛 MlflowException）
        run_env = {"MLFLOW_ALLOW_FILE_STORE": "true", **run_env}
